...
async function runChecks() {
  ...
  const wellKnownFiles = ['/.well-known/security.txt', '/.well-known/humans.txt', '/.well-known/robots.txt'];
  ...
  const report = {
    ...
    summary: {
      ...
      avg_response_time: results.filter(r => r.duration > 0).reduce((a, b) => a + b.duration, 0) / results.filter(r => r.duration > 0).length || 0
    }
  };
