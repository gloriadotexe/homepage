...
class EncryptedAuditTrail {
  ...
  verifyEntryIntegrity(entry) {
    ...
    const calculatedChecksum = crypto.createHash('sha256').update(entryString).digest('hex');
  ...
  async cleanupOldLogFiles() {
    try {
      ...
      const logFiles = files
        ...
        .sort(
          (a, b) =>
            new Date(b.timestamp.replace(/-/g, ':')) - new Date(a.timestamp.replace(/-/g, ':')),
        );
  ...
  async generateIntegrityReport() {
    ...
    try {
      ...
      const logFiles = files.filter(
        (file) => file.startsWith('security-audit-') && file.endsWith('.log'),
      );
