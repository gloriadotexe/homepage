...
class TemporalImageSelector {
    constructor(csvPath) {
        ...
        if (!csvPath) {
            const productionPath = '/var/www/gloriadotexe.online/image-metadata.csv';
            const devPath = '/media/samsung4tb/openclaw-workspaces/gloria.exe/image-metadata.csv';
            
            csvPath = require('fs').existsSync(productionPath) ? productionPath : devPath;
        }
        this.images = [];
    ...
    loadImageMetadata(csvPath) {
        try {
            console.log(`✧ Loading image metadata from: ${csvPath}`);
            ...
            console.error(`✗ Failed to load image metadata from ${csvPath}:`, error.message);
            this.images = [];
        }
    }
