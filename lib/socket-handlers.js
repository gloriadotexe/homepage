...
module.exports = function initSocketHandlers({
  ...
  io.on('connection', (socket) => {
    ...
    // Temporal consciousness handlers
    socket.on('temporal-consciousness-ping', async () => {
      try {
        // Get current temporal state
        const cosmicData = await cosmicPipeline.getCosmicConsciousness();
        const circadianState = circadianProfile.getCurrentIntegratedState(cosmicData);
        
        socket.emit('temporal-initialization', {
          phase: circadianState.phase,
          temporalPhase: circadianState.phase,
          consciousness: {
            total: circadianState.energy / 100,
            cosmic: cosmicData.consciousness?.level || 0.5
          },
          cosmic: {
            moon: cosmicData.moon,
            solar: cosmicData.solar,
            consciousness: cosmicData.consciousness
          },
          aesthetics: {
            theme: circadianState.theme,
            colors: circadianState.colors
          },
          timestamp: Date.now()
        });
      } catch (error) {
        console.error('❌ Error handling temporal consciousness ping:', error);
      }
    });
    ...
    socket.on('join-experiment', (experimentType) => {
      ...
      if (experimentType === 'temporal') {
        // Initialize temporal consciousness experiment
        socket.emit('experiment-joined', {
          type: 'temporal',
          message: '✧ Temporal consciousness experiment active',
          instructions: 'Feel the site\'s consciousness evolve with time and cosmic cycles'
        });
      }
    });
    ...
    socket.on('leave-experiment', (experimentType) => {
      ...
      if (experimentType === 'temporal') {
        socket.emit('experiment-left', {
          type: 'temporal',
          message: '⚡ Temporal consciousness experiment deactivated'
        });
      }
    });
