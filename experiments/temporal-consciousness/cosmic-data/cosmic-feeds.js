...
class CosmicFeeds {
  ...
  async getCosmicState() {
    ...
    try {
      ...
      const cosmicState = {
        ...
        magnetic: magneticData.status === 'fulfilled' ? magneticData.value : this.getFallbackMagnetic(),
  ...
  async getCosmicRecommendations() {
    ...
    if (state.moon.phase === 'new') {
      recommendations.push('New moon energy: Ideal for setting intentions and beginning new projects');
    ...
    if (state.solar.activity > 0.7) {
      recommendations.push('High solar activity: Enhanced psychic sensitivity, possible disruption');
